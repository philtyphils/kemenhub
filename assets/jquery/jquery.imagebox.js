...
(function($){
  $.fn.imageBox = function(options){
    ...
    $(objClicked).on('click', function(){
      var _url = $(this).data("url"), current = 0, _filename = $(this).data("filename");
      // 清空数组 list_images
      ...
      if(typeof(fileName) == 'undefined'){
		...
		$('#unbind-pos').find('.modal-title').text(_filename);
      }else{
	  ...
	  $('#img-preview').html('<img src="'+ _url +'" width="665px" height="440px" class="image-box" style="cursor: move;"></img>');
  ...
  */
  function initHtml(obj){
    var div = $('<div id="unbind-pos" class="modal fade bs-example-modal-lg" style="display:none; z-index:1067;" aria-hidden="true"  role="dialog"></div>'); 
    div.append('<div class="modal-dialog modal-lg">' +
                  '<div class="modal-content">'+
                        '<div class="modal-header">'+
                            '<button aria-hidden="true" data-dismiss="modal" class="close" type="button"><span>&times;</span></button>'+
                            '<h4 class="modal-title"></h4>'+
                        '</div>'+
                        '<div style="min-height: 350px;max-height: 700px;" class="modal-body">'+
                            '<div id="img-preview" class="container-fluid img-previewx"></div>'+
                            '<div class="img-op">'+
                                '<span id ="zoomiin" class="btn btn-primary zoom-in">Zoom In</span>'+
                                '<span id ="zoomoout" class="btn btn-primary zoom-out">Zoom Out</span>'+
                                '<span class="btn btn-primary rotate">Rotate</span>'+
                                '<br>'+
                                '<span id ="nextt" role="prev" class="btn btn-primary switch">Prev</span>'+
                                '<span id ="backk" role="next" class="btn btn-primary switch">Next</span>'+
                            '</div>'+
                        '</div>'+
